...
class GetTotalsModelInputSpec(SPMCommandInputSpec):
    in_image = File(exists=True, desc='moludated warped image to analyse', copyfile=False, mandatory=True)
    in_mask = File(exists=True, desc='binary mask to use', copyfile=False, mandatory=False)
    #threshold still not implemented
...
class GetTotals(SPMCommand):
    ...
    --------
    >>> gett = GetTotals()
    >>> gett.inputs.in_image = 'structural.nii'
    >>> gett.inputs.in_mask = 'mask.nii'
    >>> out = gett.run() # doctest: +SKIP
    >>> print out.outputs.total_volume # doctest: +SKIP
    """
...
"""
    ...
    def aggregate_outputs(self, runtime=None):
        ...
        setattr(outputs, 'subject', os.path.splitext(os.path.basename(self.inputs.in_image))[0])
        setattr(outputs, 'mask', os.path.splitext(os.path.basename(self.inputs.in_mask))[0])
        return outputs
